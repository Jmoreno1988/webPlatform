...
function HandlerGameChess(io, socket, modelUser, modelGames, sq, usersManager) {
	this.io = io;
	this.socket = socket;
					...
					game.updateAttributes({
							...
							// Idfollow OR idcreator del rival
								...
								if(rivalId != null) {
									...
									var conn = this.usersManager.getConnectionByIdUser(rivalId)
									if(conn){
										this.io.to(conn.socket.id).emit('moveChessPiece', {raw: raw});
									}
								}
